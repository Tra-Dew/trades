...
// DispatchTradeOfferAccepted ...
func DispatchTradeOfferAccepted(command *cobra.Command, args []string) {
	...
	pendingTrades, err := container.TradeRepository.GetByStatus(ctx, trades.TradeAccepted)
	if err != nil {
		logrus.
			WithError(err).
	...
	for _, trade := range pendingTrades {
		...
		event := trades.ParseTradeToTradeOfferAcceptedEvent(trade)

		messageID, err := container.Producer.Publish(settings.Events.TradeAccepted, event)
		...
		trade.UpdateStatus(trades.TradeAwaitingItemsTrade)
